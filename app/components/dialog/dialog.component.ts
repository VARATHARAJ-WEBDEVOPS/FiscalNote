...
export class DialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.balinput === 0) {
      ...
      this.amtinhand = Intl.NumberFormat('en-IN', { maximumSignificantDigits: 3 }).format(
        this.data.formValues.amtPerMonth
      );
  ...
  printBill() {
    ...
    const finalamt = Intl.NumberFormat('en-IN', { maximumSignificantDigits: 3 }).format(
      this.data.formValues.finalamt
    );

    const baldue = this.data.formValues.balanceMonth;

    const amtpermon = Intl.NumberFormat('en-IN', { maximumSignificantDigits: 3 }).format(
      this.data.formValues.amtPerMonth
    );
    ...
    printWindow.document.write(
      '<tr><td>Received Amount: </td><td>₹ ' + amount + '/-</td></tr><br/>'
    );
    ...
    printWindow.document.write(
      '<tr><td>Total amount due </td><td> ₹ ' + finalamt + '/-</td></tr><br/>'
    );
